(() => {
	...
	function resolveAIActionsAsync(onComplete){
		...
		function step(){
			...
			for(let idx=0; idx<2; idx++){
				...
				if(toCall === 0){
					if((seat.raiseCount||0) < 2 && Math.random() < 0.12 && seat.stack > 20){
					...
					if(toCall <= Math.min(Math.ceil(seat.stack*0.15), 50)){
	...
	// Win message helper
	function formatWinMessage(name, amount){
		if(name === 'You') return `You win ${amount} chips!`;
		return `${name} wins ${amount} chips!`;
	}
